...
class KernelSpatialModulation_Global(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, groups=1, reduction=0.0625, kernel_num=4, min_channel=16, 
                 temp=1.0, kernel_temp=None, kernel_att_init='dyconv_as_extra', att_multi=2.0, ksm_only_kernel_att=False,
        ...
        self.ksm_only_kernel_att = ksm_only_kernel_att  # 仅使用核注意力
        ...
        self.att_grid = att_grid    # 注意力网络的分辨率
        ...
        if ksm_only_kernel_att:
            ...
            self.func_channel = self.get_channel_attention  # 通道注意力
        ...
        if (in_planes == groups and in_planes == out_planes) or self.ksm_only_kernel_att:  # depth-wise convolution
            ...
            self.func_filter = self.get_filter_attention    # 滤波器注意力

        if kernel_size == 1 or self.ksm_only_kernel_att:  # point-wise convolution
            self.func_spatial = self.skip
        else:
            self.spatial_fc = nn.Conv2d(attention_channel, kernel_size * kernel_size, 1, bias=True)
            self.func_spatial = self.get_spatial_attention  # 空间注意力
        ...
        if kernel_num == 1:
            ...
            self.func_kernel = self.get_kernel_attention    # 核注意力
