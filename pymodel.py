...
class pyCNN(nn.Module):
    def __init__(self,Classes,FM=64,NC=30,para_tune=True):
        ...
        #     # 修改1：将输入通道从1扩展到FM//4（可调）
        #     nn.Conv2d(1, FM // 4, kernel_size=1),  # 通道扩展层
        #
        #     # 修改2：FDConv输入输出通道改为FM//4
        #     FDConv(
        #         in_channels=FM // 4,
        #         out_channels=FM // 4,
        #         kernel_size=(2, 1),
        ...
        #     nn.ReLU(),
        #
        #     # 修改3：调整Flatten后的线性层输入维度
        #     nn.Flatten(),
        #     nn.Linear(FM // 4 * 256, Classes)  # feature_dim需要计算
        # )


        # add融合
        self.out3 = nn.Sequential(
            nn.Conv2d(1, FM // 4, kernel_size=1),  # 输入通道=1
            FDConv(
                in_channels=FM // 4,
                out_channels=FM // 4,
                kernel_size=(2, 1),
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(2048, Classes)  # 直接使用实际维度 2048
        )
